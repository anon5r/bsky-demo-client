...
すべてのレスポンスは、特別な指示がある場合を除き、すべて日本語で行います。

コードの品質を維持するため、以下のルールを遵守してください。

1.  **ビルドとLintの確認**:
    コードを変更した後は、必ず以下のコマンドを実行してエラーがないことを確認してください。
    - ビルド確認: `npm run build`
    - Lint確認: `npm run lint`

2.  **コードフォーマット**:
    コードを変更または生成する際は、適切なインデントとフォーマットを適用してください。プロジェクトにPrettierなどのフォーマッターが導入されている場合はそれを使用し、そうでない場合も可読性の高いコード記述を心がけてください。

3.  **Gitコミット**:
    ファイルを更新・修正した後は、必ずGitコミットを行ってください。コミットメッセージは変更内容（Why/What）を明確かつ簡潔に記述してください。
...
4. ✅ 即時投稿（単一 & スレッド）と各種リアクション、通知表示、投稿の削除、およびChronoskyと連携したスケジュール投稿の予約
